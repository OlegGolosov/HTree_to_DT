    ...
    TString parameterFile = "../evtchara07/centrality_epcorr_apr12_gen8_2019_02_pass30.root")
{
  ...
  if(!isSimulation)
    loop.readSectorFileList("/lustre/nyx/hades/dst/apr12/gen8/sector_selection/FileListHadron.list", kFALSE, kFALSE);
  ...
  for(Int_t i = 1; i < entries; i++)
  {
    ...
    DTEvent->SetVertexPosition(
        vertexReco.getX(), vertexReco.getY(), vertexReco.getZ(), EnumVertexType::kReconstructedVertex);
    ...
    for(auto estimator : centralityEstimatorMap)
    {
      ...
      DTEvent->SetCentralityClass(
          estimator.first, evtChara.getCentralityClass(estimator.second, HParticleEvtCharaBK::k5));
    }
    ...
    for(Int_t j = 0; j < size; j++)
    {
      ...
      if(cand->getSystem() == 0)
        hit->SetStatus(HADES_constants::kRPC);
      else
        hit->SetStatus(HADES_constants::kTOF);
      hit->SetTime(cand->getDistanceToMetaHit() / cand->getBeta() / 299.792458);
      hit->SetPathLength(cand->getDistanceToMetaHit());
      hit->SetPosition(
          cand->getRkMetaDx(), cand->getRkMetaDy(), cand->getMetaMatchRadius()); // META match qa - NOT POSITION!!!
